...
def get_page_urls(page_source, url):
    ...
    urls_2 = [urlparse(urljoin(url, f"?page={o['value']}"))
              for o in options_2[1:] if o]
...
def get_article_urls(page_source, url):
    ...
    return [urlparse(urljoin(url, td.find('a')['href'])) for td in article_titles if td.find('a')]


@ArticleSoup.register('title')
def article_title(soup):
    article_container = soup.find('article', {'class': 'post'})
    title = article_container.find(
        attrs={'class': 'title'}) if article_container else None
...
def publish_date(soup):
    ...
    date_3_text = date_3.get_text(
        strip=True, separator=' ') if date_3 else None
...
def article_text(soup):
    ...
    content = article_container.find(
        'div', {'class': 'content'}) if article_container else None
    if content:
