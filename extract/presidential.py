...
def get_page_urls(page_source):
    ...
    first_page = re.search(r'\d+', pages[0].text).group()
    last_page = re.search(r'\d+', pages[-1].text).group()
...
def article_type(soup):
    ...
    return cat_tag.get_text(strip=True, separator=' ')
...
def extract_from_file(files:list):
    ...
    save_extract({'records': extracted}, 'presidential_extract')
...
def save_extract(extracted, filename):
    ...
    if len(filename) > 225:
        filename = filename[:-(225-len(filename))]
