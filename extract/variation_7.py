...
def get_article_urls(page_source):
    ...
    return [urlparse(urljoin(URL, div.find('a')['href'])) for div in article_containers if div.find('a')]
...
def publish_date(soup):
    date_container = soup.find('div', {'class': 'evo-create-type'})
    date = date_container.find('div', {'class': 'col-auto'})
...
def article_text(soup):
    content = soup.find('div', {'class':['evo-article__body',
...
def extract_from_file(files:list):
    ...
    save_extract({'records': extracted}, 'variation-7_extract')
    return extracted
...
def main():
    ...
    listing_p_bar = tqdm(total=0, desc='Article listing iterated...')
    articles_p_bar = tqdm(total=0, desc='Articles gathered...')

    while True:
        try:
            p_link = urlparse(urljoin(URL, f'?page={listing_p_bar.total}'))
    ...
    #         p_link = urlparse(urljoin(URL, f'?page={page_counter}'))
    #         chrome_driver.get(p_link.geturl())
