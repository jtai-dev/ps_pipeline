...
def get_page_urls(page_source, url):
    ...
    next_page_link = (
        urlparse(next_page.find("a")["href"])
        if next_page and next_page.find("a")
        else None
    )
    last_page_link = (
        urlparse(last_page.find("a")["href"])
        if last_page and last_page.find("a")
        else None
    )

    first_page_num = (
        int(next_page_link.query.split("=")[-1]) - 1 if next_page_link else 1
    )
...
def article_text(soup):
    ...
    content = (
        article_container.find("div", {"class": "field-items"})
